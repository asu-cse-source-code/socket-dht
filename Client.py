...
class Client:
    ...
    def create_sockets(self):
        ...
        self.client_left_port = UDPServer()
        self.client_query = UDPServer()
        self.client_right_port = UDPServer()
...
def check_record(client, record):
    ...
    if id == client.id:
        ...
        # Could turn each of these record send commands into threads :)
        while client.record:
            # Sleep to allow the records to be sent
            time.sleep(0.5)
...
def initialize_client_topology(client):
    ...
    try:
        client_server.socket.bind(client.left_port_addr)
